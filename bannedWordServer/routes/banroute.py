...
class BanRoute(Resource):
	...
	def post_one(self, session, authToken, serverid: str, banid: str, banned_word: str) -> dict:
		...
		ban_to_modify = session.query(Ban).filter_by(server_id=serverid, rowid=banid).first()
		...
		ban_to_modify.calledout_at = (datetime.now() - timedelta(weeks=52)).strftime("%Y-%m-%d %H:%M:%S")
