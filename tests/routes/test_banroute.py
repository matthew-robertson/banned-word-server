...
class TestBanRouteGetCollection(TestCase):
    ...
    def test_banroute_get_collection__one_element(self):
        ...
        result = BanRoute().get_collection(
            self.session, "Bot " + BOT_TOKEN, self.serverid
        )
...
class TestBanRouteGetOne(TestCase):
    def setUp(self):
        db.Model.metadata.create_all(engine)
        self.authtoken = "Bot " + BOT_TOKEN
        self.connection = engine.connect()
    ...
    def test_banroute_get_one__bad_serverid(self):
        self.assertRaises(
            InvalidTypeError, BanRoute().get_one, self.session, self.authtoken, "asdf"
        )

    def test_banroute_get_one__not_found(self):
        self.assertRaises(
            NotFoundError, BanRoute().get_one, self.session, self.authtoken, "0"
        )
...
class TestBanRoutePostCollection(TestCase):
    ...
    def test_banroute_post_collection__good_request(self):
        banned_word = "asdf"
        result = BanRoute().post_collection(
            self.session, self.authtoken, self.serverid, banned_word
        )
        db_result = (
            self.session.query(Ban)
            .filter_by(server_id=self.serverid, banned_word=banned_word)
            .first()
        )
    ...
    def test_banroute_post_collection__duplicate_word(self):
        banned_word = "asdf"
        BanRoute().post_collection(
            self.session, "Bot " + BOT_TOKEN, self.serverid, banned_word
        )
        self.assertRaises(
            DuplicateResourceError,
            BanRoute().post_collection,
    ...
    def test_banroute_post_collection__confusable_word(self):
        BanRoute().post_collection(
            self.session, "Bot " + BOT_TOKEN, self.serverid, "lest"
        )
        self.assertRaises(
            DuplicateResourceError,
            BanRoute().post_collection,
...
class TestBanRoutePostOne(TestCase):
    def setUp(self):
        ...
        new_ban = Ban(
            ...
            banned_word="asdf",
            infracted_at=(datetime.now() - timedelta(days=6)).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
        )
        self.record = BanRecord(
            server_banned_word=new_ban, record_seconds=60, infraction_count=420
        )
        ...
        record1 = BanRecord(
            server_banned_word=ban1, record_seconds=60, infraction_count=420
        )
        ban2 = Ban(
            server_id=self.overloaded_server_id,
            banned_word="fdsa",
        ...
        record2 = BanRecord(
            server_banned_word=ban2, record_seconds=60, infraction_count=420
        )
        self.session.add(ban1)
        self.session.add(record1)
        self.session.add(ban2)
    ...
    def test_banroute_post_one__good_request(self):
        ...
        self.assertEqual(
            new_word,
            BanRoute().get_one(self.session, "Bot " + BOT_TOKEN, banid)["banned_word"],
        )
        self.assertEqual(
            True,
...
class TestBanRouteDelete(TestCase):
    ...
    def test_banroute_delete__good_request(self):
        ...
        self.assertRaises(
            NotFoundError, BanRoute().get_one, self.session, "Bot " + BOT_TOKEN, banid
        )
        server_words = (
            self.session.query(Server)
            .filter_by(server_id=self.serverid)
            .first()
            .banned_words
        )
