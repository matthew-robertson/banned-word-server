...
class TestBanRouteGetCollection(TestCase):
	...
	def test_banroute_get_collection__unauthorized(self):
		self.assertRaises(AuthenticationError, BanRoute().get_collection, self.session, "asdffdsa", 12345)
	...
	def test_banroute_get_collection__one_element(self):
		...
		result = BanRoute().get_collection(self.session, BOT_TOKEN, self.serverid)
...
class TestBanRouteGetCollection(TestCase):
	...
	def test_banroute_get_one__unauthorized(self):
		self.assertRaises(AuthenticationError, BanRoute().get_one, self.session, "asdffdsa", "0")
	...
	def test_banroute_get_one__good_request(self):
		...
		result = BanRoute().get_one(self.session, BOT_TOKEN, "2")
...
class TestBanRoutePostCollection(TestCase):
	...
	def test_banroute_post_collection__unauthorized(self):
		self.assertRaises(AuthenticationError, BanRoute().post_collection, self.session, "asdffdsa", self.serverid, "asdf")

	def test_banroute_post_collection__duplicate_word(self):
		banned_word = "asdf"
		BanRoute().post_collection(self.session, BOT_TOKEN, self.serverid, banned_word)
	...
	def test_banroute_post_collection__confusable_word(self):
		BanRoute().post_collection(self.session, BOT_TOKEN, self.serverid, 'lest')
...
class TestBanRoutePostOne(TestCase):
	...
	def test_banroute_post_one__good_request(self):
		...
		BanRoute().post_one(self.session, BOT_TOKEN, self.serverid, banid, new_word)
		self.assertEqual(new_word, BanRoute().get_one(self.session, BOT_TOKEN, banid)['banned_word'])
	...
	def test_banroute_post_one__ban_not_found(self):
		self.assertRaises(NotFoundError, BanRoute().post_one, self.session, BOT_TOKEN, self.serverid, 5, "asdf")

	def test_banroute_post_one__confusable_word(self):
		BanRoute().post_collection(self.session, BOT_TOKEN, self.serverid, 'lest')
		self.assertRaises(DuplicateResourceError, BanRoute().post_one, self.session, BOT_TOKEN, self.serverid, 1, "iest")

	def test_banroute_post_one__unauthorized(self):
		self.assertRaises(AuthenticationError, BanRoute().post_one, self.session, "asdffdsa", self.serverid, 5, "asdf")

	def test_banroute_post_one__word_invalid(self):
		self.assertRaises(InvalidTypeError, BanRoute().post_one, self.session, BOT_TOKEN, self.serverid, 1, 1234)
...
class TestBanRouteDelete(TestCase):
	...
	def test_banroute_delete__good_request(self):
		...
		BanRoute().delete(self.session, BOT_TOKEN, self.serverid, banid)
		self.assertRaises(NotFoundError, BanRoute().get_one, self.session, BOT_TOKEN, banid)
	...
	def test_banroute_delete__ban_not_found(self):
		self.assertRaises(NotFoundError, BanRoute().delete, self.session, BOT_TOKEN, self.serverid, 5)

	def test_banroute_delete__unauthorized(self):
		self.assertRaises(AuthenticationError, BanRoute().delete, self.session, "asdffdsa", self.serverid, 1)

	def test_banroute_delete__twice(self):
		banid = 1
		BanRoute().delete(self.session, BOT_TOKEN, self.serverid, banid)
		self.assertRaises(NotFoundError, BanRoute().delete, self.session, BOT_TOKEN, self.serverid, banid)

	def test_banroute_delete__delete_last_word(self):
		BanRoute().delete(self.session, BOT_TOKEN, self.serverid, 1)
		BanRoute().delete(self.session, BOT_TOKEN, self.serverid, 3)
		self.assertRaises(ValidationError, BanRoute().delete, self.session, BOT_TOKEN, self.serverid, 2)
